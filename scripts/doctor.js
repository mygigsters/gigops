...
check('Playwright browsers', () => {
    try {
        (0, child_process_1.execSync)('npx playwright install chromium --dry-run 2>&1', { stdio: 'pipe' });
...
// Check profile
check('User profile', () => {
    const profilePaths = ['./config/profile.yml', path_1.default.join(process.env.HOME || '~', '.gigops', 'profile.yml')];
...
check('Platform configs', () => {
    ...
    const missing = platforms.filter(p => !fs_1.default.existsSync(`./config/platforms/${p}.yml`));
    if (missing.length === 0) {
...
check('Pipeline directory', () => {
    if (fs_1.default.existsSync('./pipeline')) {
        return { name: 'Pipeline directory', status: 'pass', message: 'pipeline/ exists ✓' };
    }
    fs_1.default.mkdirSync('./pipeline', { recursive: true });
    fs_1.default.writeFileSync('./pipeline/.gitkeep', '');
...
for (const r of results) {
    if (r.status === 'pass') {
        console.log(chalk_1.default.green(`  ✓ ${r.name.padEnd(25)} ${r.message}`));
        passCount++;
    }
    else if (r.status === 'warn') {
        console.log(chalk_1.default.yellow(`  ⚠ ${r.name.padEnd(25)} ${r.message}`));
        if (r.fix)
            console.log(chalk_1.default.dim(`      Fix: ${r.fix}`));
        warnCount++;
    }
    else {
        console.log(chalk_1.default.red(`  ✗ ${r.name.padEnd(25)} ${r.message}`));
        if (r.fix)
            console.log(chalk_1.default.dim(`      Fix: ${r.fix}`));
        failCount++;
    }
}
console.log(`\n  ${chalk_1.default.green(`${passCount} passed`)}  ${chalk_1.default.yellow(`${warnCount} warnings`)}  ${chalk_1.default.red(`${failCount} failed`)}\n`);
if (failCount > 0) {
    console.log(chalk_1.default.red('  ✗ Setup incomplete. Fix the issues above before using GigOps.\n'));
    process.exit(1);
}
else if (warnCount > 0) {
    console.log(chalk_1.default.yellow('  ⚠ Setup mostly complete. Address warnings for full functionality.\n'));
}
else {
    console.log(chalk_1.default.green('  ✓ GigOps is ready to go! Run `gigops --help` to get started.\n'));
}
